<x-layouts.app :title="'Kelola Produk - '.$product->name">
    <div class="space-y-6 p-6">
        ...
        <div class="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div class="space-y-6">
                <div class="rounded-2xl border border-neutral-100 bg-white p-5 sm:p-6">
                    <h2 class="mb-4 text-sm font-bold text-zinc-900">Gambar produk</h2>

                    <form method="POST" action="{{ route('admin.products.media.store', $product) }}" enctype="multipart/form-data" class="mb-5 rounded-xl border-2 border-dashed border-neutral-200 p-4">
                        @csrf
                        <label for="image" class="mb-2 block text-xs font-semibold text-zinc-700">Upload gambar baru</label>
                        <input
                            id="image"
                            name="image"
                            type="file"
                            ...
                            required
                            class="block w-full text-sm text-zinc-600 file:mr-3 file:rounded-xl file:border-0 file:bg-zinc-100 file:px-3 file:py-2 file:text-xs file:font-bold file:text-zinc-700 hover:file:bg-zinc-200"
                        >
                        <div class="mt-3 flex flex-wrap items-center justify-between gap-2">
                            <label class="flex cursor-pointer items-center gap-1.5 text-xs text-zinc-600">
                                <input type="checkbox" name="is_primary" value="1" class="rounded border-zinc-300">
                                Jadikan gambar utama
                            </label>
                            <button type="submit" class="rounded-xl bg-zinc-900 px-4 py-2 text-xs font-bold text-white transition hover:bg-zinc-700">
                                Upload
                            </button>
                        </div>
                    </form>
                    ...
                    @if ($product->media->isEmpty())
                        <p class="py-6 text-center text-xs text-zinc-400">Belum ada gambar. Upload gambar utama untuk thumbnail katalog.</p>
                    @else
                        <div class="grid grid-cols-2 gap-3">
                            @foreach ($product->media as $media)
                                <div class="overflow-hidden rounded-xl border border-neutral-100">
                                    <img
                                        src="{{ route('admin.products.media.show', $media) }}"
                                        alt="{{ $media->alt_text }}"
                                    ...
                                    <div class="flex items-center justify-between gap-1 border-t border-neutral-100 px-2.5 py-2">
                                        @if ($media->is_primary)
                                            <span class="rounded-full bg-brand-yellow/20 px-2 py-0.5 text-[10px] font-bold text-brand-yellow-dark">Utama</span>
                                        @else
                                            <span class="text-[10px] text-zinc-400">Gambar</span>
                                        @endif
                                        <form method="POST" action="{{ route('admin.products.media.destroy', $media) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="rounded-lg p-1 text-zinc-400 transition hover:bg-red-50 hover:text-red-600" title="Hapus gambar">
